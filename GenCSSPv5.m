...
export_path = './graphsv5'; % Directory to save if plot_mode = 1
sf=1; % 1 for save C

%% --- Parámetros de Video ---
% 0: No generar frames de video (más rápido).
% 1: Generar un frame solo cuando se encuentra una mejor configuración.
% 2: Generar un frame en cada iteración (¡puede ser lento y usar mucho disco!).
video_mode = 1; % Cambiar a 1 o 2 para activar
video_export_path = './video_frames/sspv5'; % Carpeta para guardar los frames

% --- Configuración de Archivo de Log ---
log_filename = 'summary_log_v5.txt';
fileID = fopen(log_filename, 'w'); % Open in write mode to clear on each run
fprintf(fileID, 'Execution Summary - %s\n\n', datetime('now'));

mux_values_to_run = 1:5;
...
for m_idx = 2:length(mux_values_to_run)
    ...
    %% --- Generación del Frame Inicial para Video ---
    if video_mode > 0
        if ~exist(video_export_path, 'dir')
            mkdir(video_export_path);
        end
        fprintf('>> Guardando frame inicial (iter 0) para M=%d...\n', M);
        % Usamos R_mode = 0 para que las esferas iniciales sean del mismo tamaño (regulares)
        plotSphereFrame(C_initial, 0, 0, M, initial_cond, initial_density, video_export_path);
    end
    ...
    if M > 1
        ...
        if video_mode > 0 && ~exist(video_export_path, 'dir')
            mkdir(video_export_path);
        end
        ...
        for k = 1:max_iterations
            plot_this_frame = false; % Flag to decide whether to plot this frame
            ...
            if plot_this_frame
                plotSphereFrame(C, R, k, M, new_cond, new_density, video_export_path);
                if video_mode == 1 % Notify only when saving due to improvement
                     fprintf('>> Video frame (improvement) saved for M=%d, iter=%d.\n', M, k);
                end
            end
        ...
        if plot_mode > 0
            ...
            if show_sphere_plots
                showSphereComparison(C_initial, C_optimized, C_uniformized, M, fig_vis, export_path);
            end
        end
    else
