...
log_filename = 'summary_log_gpu.txt';
fileID = fopen(log_filename, 'w'); % Open in write mode to clear on each run
fprintf(fileID, 'Execution Summary (GPU) - %s
...
');
    ...
    fprintf('GPU not available or not selected. Running on CPU.
');
    use_gpu = false;
end
...
');
        ...
        for k = 1:max_iterations
            % Assuming uniqueDistanceIrregularSP is a CPU-only function
            C_cpu = gather(C);
            [list, diameters] = uniqueDistanceIrregularSP(C_cpu);
...
');
            ...
            CNr(k) = gather(new_cond);
            sumden(k) = gather(new_density);
...
', k);
        ...
        if plot_mode > 0
            ...
            hold on; 
            yline(gather(best_cond), '--r', ['Min: ' num2str(gather(best_cond), '%.2f')], 'LabelVerticalAlignment', 'bottom');
            ...
            hold on;
            yline(gather(best_density), '--r', ['Max: ' num2str(gather(best_density), '%.4f')], 'LabelVerticalAlignment', 'bottom');
...
', M, filename);
        ...
        sum_x_opt = reshape(sum(gather(C_optimized), [2, 3]), [N, 1]);
        sum_y_opt = reshape(sum(gather(C_optimized), [1, 3]), [N, 1]);
        sum_z_opt = reshape(sum(gather(C_optimized), [1, 2]), [N, 1]);
...
');
        ...
        % Apply uniformizer
        best_C = uniformizeMatrix(best_C, M, use_gpu);
...
', gather(final_cond), gather(density_after));

        %% --- UNIFORMITY ANALYSIS ---
        % Sums by vectors (lines) for text report
        sum_lines_x = reshape(sum(gather(best_C), [2, 3]), [N, 1]);
        sum_lines_y = reshape(sum(gather(best_C), [1, 3]), [N, 1]);
        sum_lines_z = reshape(sum(gather(best_C), [1, 2]), [N, 1]);
...
', ...
        ...
        if plot_mode > 0
            ...
            s_b_yz = squeeze(sum(gather(C_before), 1));
            s_b_xz = squeeze(sum(gather(C_before), 2));
            s_b_xy = squeeze(sum(gather(C_before), 3));
            s_a_yz = squeeze(sum(gather(best_C), 1));
            s_a_xz = squeeze(sum(gather(best_C), 2));
            s_a_xy = squeeze(sum(gather(best_C), 3));
...
', M, filename_uniformity);
            ...
            if show_sphere_plots
                ...
                subplot(1,3,2);
                showSphereCA(gather(C_optimized), 12, 1); % op=1 for irregular
                title(['Optimizado (Cond=' num2str(gather(best_cond), '%.2f') ')'], 'Color', 'k');

                % Subplot 3: Uniformized State (Irregular Spheres)
                subplot(1,3,3);
                showSphereCA(gather(C_uniformized), 12, 1); % op=1 for irregular
                title(['Uniformizado (Cond=' num2str(gather(final_cond), '%.2f') ')'], 'Color', 'k');
...
', gather(initial_cond), gather(best_cond), gather(final_cond));
    fprintf('  Densidad:  Inicial: %.4f | Optimizada: %.4f | Uniforme: %.4f
', gather(initial_density), gather(best_density), gather(density_after));
...
', initial_rmse, optimized_rmse, final_rmse);
    ...
    if sf==1
        ...
        initial_pattern.condition_number = gather(initial_cond);
        initial_pattern.density = gather(initial_density);
        initial_pattern.uniformity_rmse = initial_rmse;
        
        optimized_pattern.matrix = gather(C_optimized);
        optimized_pattern.condition_number = gather(best_cond);
        optimized_pattern.density = gather(best_density);
        optimized_pattern.uniformity_rmse = optimized_rmse;
        
        uniformized_pattern.matrix = gather(C_uniformized);
        uniformized_pattern.condition_number = gather(final_cond);
        uniformized_pattern.density = gather(density_after);
    ...
    summary_initial_cond(m_idx) = gather(initial_cond);
    summary_optimized_cond(m_idx) = gather(best_cond);
    summary_uniform_cond(m_idx) = gather(final_cond);
    summary_initial_density(m_idx) = gather(initial_density);
    summary_optimized_density(m_idx) = gather(best_density);
    summary_uniform_density(m_idx) = gather(density_after);
...
function C_out = uniformizeMatrix(C_in, M, use_gpu)
    ...
    for iter = 1:max_iter
        ...
        % 6. Perform a random swap
        rand_one_idx = randi(length(movable_ones_indices));
        rand_zero_idx = randi(length(fillable_zeros_indices));

        one_to_move_flat_idx = movable_ones_indices(rand_one_idx);
